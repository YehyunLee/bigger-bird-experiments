...
Validates whether each sparse-attention method preserves *genuine* long-range signal
(unlike padded IMDb), across a range of context windows. Mirrors run_experiment.py.

Usage:
  python run_lra_experiment.py --task listops --exp 0 --seq 2048
  python run_lra_experiment.py --task text --exp 1 --seq 4096 --size lra-full
  python run_lra_experiment.py --task retrieval --exp 7 --seq 4096 --data-dir lra_data/retrieval
  python run_lra_experiment.py --list
"""
...
# Compute presets for the from-scratch LRA encoder.
LRA_COMPUTE = {
    "lra-smoke": {
        "train_samples": 256,
        "eval_samples": 128,
...
# Default context window per task (genuine long-range, not padding).

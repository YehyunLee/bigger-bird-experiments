...
# Idea F: Grouped-Query Attention (GQA) + Sparse Top-K Routing
# Two memory wins stacked together:
#   1. GQA: 12 query heads share only `kv_groups` (e.g., 4) sets of K/V via mean pooling.
#      KV cache shrinks by 12 / kv_groups (e.g., 3x).
#   2. Sparse routing: DeepSeek-style top-K routing on top of GQA.
#      Softmax cost per query drops from O(N) to O(K).
# Implementation: keep BART's existing weights, but AT RUNTIME average K and V
# across head-groups before the attention computation. No new params required.
...
class GQASparseAttention(BartAttention):
    def __init__(self, base_attn: BartAttention,
        ...
        assert self.num_heads % kv_groups == 0, f"num_heads ({self.num_heads}) must be divisible by kv_groups ({kv_groups})"
    ...
    def _gqa_compress(self, k_or_v, bsz, seq_len):
        """Compress K or V from [B, H, T, D] to [B, G, T, D] by averaging within groups."""
    ...
    def forward(self, hidden_states, key_value_states=None, past_key_value=None,
        ...
        # GQA compress K, V: [B, G, src_len, D]
        K_g = self._gqa_compress(Ks, bsz, src_len)
        V_g = self._gqa_compress(Vs, bsz, src_len)
        # Broadcast back so each query head reads from its group's K/V
        # [B, G, src_len, D] -> [B, H, src_len, D] by repeat_interleave
        K_full = K_g.repeat_interleave(self.heads_per_group, dim=1)
        V_full = V_g.repeat_interleave(self.heads_per_group, dim=1)

        BH = bsz * self.num_heads
        Q = self._shape(Qs, tgt_len, bsz).reshape(BH, tgt_len, self.head_dim)
        K = K_full.reshape(BH, src_len, self.head_dim)
        V = V_full.reshape(BH, src_len, self.head_dim)
